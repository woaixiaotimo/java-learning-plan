    ...
    public void listen() {
        ...
        server.accept(null, new CompletionHandler<AsynchronousSocketChannel, Object>() {
            ...
            private void handle(final AsynchronousSocketChannel channel) {
                ...
                readBuffer.clear();
                //为每个连接设置单独的处理对象(异步非阻塞调用)
                channel.read(readBuffer, null, new CompletionHandler<Integer, Object>() {

                    //记录服务器接收报文次数
                    final AtomicInteger questionCount = new AtomicInteger(0);
                    ...
                    public void failed(Throwable exc, Object attachment) {
                        System.out.println("server read failed: " + exc + " 接收次数为：" + questionCount);
                        if (channel != null) {
                            try {
                                channel.close();
                            } catch (IOException e) {
